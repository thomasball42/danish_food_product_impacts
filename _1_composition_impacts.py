...
impact_file = data_dir / "mrio_food_outputs" / "DNK_TRADE2021_SPAM2010" / "food_commodity_impacts.csv" # this is for 2021, but with 2010 spam data (because 2020 is suspicious)
...
def main():
    ...
    def map_impact_err_func(x):
        # have to do this because the FAO entry for a few big things are missing for Denmark (Barley, Beans, Wheat)
        if x in missing_fix.Item.to_list():
            try:
                missing_item_info = missing_fix[missing_fix.Item == x]
    ...
    for file in comp_dir.glob("*.csv"):
        ...
        # carry groups forward to fill missing values
